...
library(survivalROC)
setwd('.07_multiROC')
rt=read.table("indepInput1.txt",header=T,sep="\t",check.names=F,row.names=1)
#rt$futime=rt$futime/365
rocCol=rainbow(ncol(rt)-2)
aucText=c()

#pdf(file="multiROC3.pdf",width=6,height=6)

par(oma=c(0.5,1,0,1),font.lab=1.5,font.axis=1.5)
#predict 1-year/3-year/5-year ROC  
#mitoriskscore prediction
...
#other clinic features prediction
j=1
